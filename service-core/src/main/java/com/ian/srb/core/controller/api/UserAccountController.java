...
public class UserAccountController {
    ...
    public Rs commitCharge(
        ...
        String formStr = userAccountService.submit(chargeAmt, userId);
    ...
    public Rs getAccount(HttpServletRequest request) {
        String token = request.getHeader("token");
        Long userId = JwtUtils.getUserId(token);
       BigDecimal account =  userAccountService.getAccount(userId);
        return Rs.success().data("account",account);
    }


    @ApiOperation("用户提现")
    @PostMapping("/auth/commitWithdraw/{fetchAmt}")
    public Rs commitWithdraw(
